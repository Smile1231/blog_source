...
{% asset_img 2022-02-09-15-44-02.png %}
...
- **作用**:对于大数据的`2`进制运算,位移运算符比那些普通运算符的运算要快很多,因为程序仅仅移动一下而已,不去计算,这样提高了效率,节省了资源 　比如这里：`int newCapacity = oldCapacity + (oldCapacity >> 1)`; 右移一位相当于除`2`，右移`n`位相当于除以 `2` 的 `n` 次方。这里 `oldCapacity` 明显右移了1位所以相当于`oldCapacity /2`。
